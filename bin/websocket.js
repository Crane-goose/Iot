...
function sendData(equipmentId, data1, data2, data3, data4, data5, data6) {
  let msg
  // 捕捉 JSON序列化时的异常
  try{
    //msg = JSON.stringify([{time:moment().format('mm:ss'),value:data}])
    msg = JSON.stringify([{time:moment().format('mm:ss'), value1:data1, value2:data2, value3:data3, value4:data4, value5:data5, value6:data6}])
  }
  catch(err){
